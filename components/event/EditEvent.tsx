...
import { Loader, Edit, Save, ChevronDown } from "lucide-react";
...
const EditEvent = ({ event }: { event: EventInterfaceType }) => {
  const { handleUpdateEvent, setIsLoading, isLoading } = EventHook();
  ...
  const form = useForm<FormValues>({
    ...
    defaultValues: {
      ...
      image: event?.image || "",
      specialDeal: event?.specialDeal || false,
      mode: event?.mode || "",
      refundPolicy: event?.refundPolicy || "",
      geolocation: {
        address: event?.geolocation?.address || "",
        city: event?.geolocation?.city || "",
        state: event?.geolocation?.state || "",
